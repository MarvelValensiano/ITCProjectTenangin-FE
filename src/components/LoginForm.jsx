...
export default function LoginForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // backend mengembalikan accessToken & user (lihat contoh responsmu)
      // dukung beberapa nama field token jika backend beda
      const token = data?.accessToken || data?.token || data?.access_token || (data?.data && (data.data.accessToken || data.data.token));
      const userObj = data?.user || data?.data?.user || { username: data?.username, email: data?.email };

      if (!token) {
        // ada respons sukses tapi tanpa token: anggap error
        throw new Error("Login sukses tapi token tidak ditemukan pada response.");
      }

      // simpan token & user
      try {
        // jika user memilih remember, simpan di localStorage; kalau tidak, tetap simpan di localStorage agar Home bisa membaca
        // (kamu bisa ubah jadi sessionStorage jika mau non-persistent)
